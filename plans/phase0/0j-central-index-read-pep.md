...
> **R5 — SOLE central-index read-PEP authority (reconciliation):** This sub-plan (`0j`) is the **single authoritative central-index read PEP**. The `CentralIndexReadPEP` class defined here — resolving scope from the owner-authoritative, strongly-consistent monotonic scope-epoch (`ScopeRegistry.current_scope`) — is the **one and only** scope-filter implementation. `0c` (pep-broker-chokepoint) MUST NOT duplicate it: `0c` drops any standalone `filter_by_discoverability` free function (or reduces it to a thin delegate to this class), so there is exactly one scope-filter codepath. `discoverability_scope` is always passed as the `DiscoverabilityScope` enum member (never a raw string) across every commit, query, response, and store call in this plan.

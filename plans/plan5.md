...
> Adversarial review of this version: **None critical**, **None high** findings.

---

This is a plan revision task — no skill matches it, and the instructions say to return findings/plans directly as my final message. I'll produce the revised plan as my response. This is a writing/synthesis task that doesn't require codebase exploration; all inputs (locked requirements, review findings, v4 plan) are in the prompt.

Let me write version 5, resolving every critical and high finding, addressing mediums where sensible, keeping all 19 sections.

# TigerExchange — Federated Multi-University Research Collaboration & Discovery Platform — Architecture & Business Plan v5

*Self-contained. The architecture is constant across phases; build order and isolation posture are the levers. This revision derives the load-bearing distributed-systems and security numbers instead of asserting them, makes confidentiality a mediated chokepoint rather than a per-module contract, reconciles unit economics against COGS, and decomposes the federation program into shippable, separately-validated sub-phases.*
...
- **Research office (VP Research / sponsored programs)** — top-down institutional buyer; owns compliance, grant strategy; **6+ month committee cycle with veto chains.**
...
### 2.2 The four (five) buyers, honestly mapped (resolves GTM-high-four-buyers-contradictory, GTM-low-pricing-metric)
...
### 2.5 GTM sequencing
Phase 0 beachhead (NDA lab/clinical center, paid sandbox) → Phase 1 first federation **dyad sold into a pre-existing confidential-sharing relationship** (resolves cold-start, §3.4) → Phase 2 institutional + consortium → Phase 3 PLG brand-surface + grant monetization.
...
### 3.3 Two independent kill-gates (resolves GTM-crit-wedge-no-moat, COST-crit-mvp-scope-economics)
...
### 3.4 Cold-start: sell into a pre-existing relationship (resolves GTM-crit-cold-start)
...
- **PSI collaborator-overlap** → Phase 2, but **usability spiked in Phase 1** (resolves DS-low-PSI).
...
### 4.2 The single confidentiality chokepoint (resolves MOD-crit-confidentiality-smeared)
**Every** retrieval, egress, and derivation in a cell flows through **one Policy Enforcement Point + Data-Access Broker.** Feature modules receive **already-projected, already-tier-checked result objects** and **never see raw classification logic or the raw store.** This is enforced structurally:
...
**Confidential collaboration — brokered, owner-authoritative drill-down (resolves SEC-high-confused-deputy, DS-med-brokered-budget).**
...
**Per-hop deadline-propagated latency budget for the brokered confidential drill-down (resolves DS-med-brokered-budget; composite SLO = 6.5s):**
...
**Latency derivation (replaces the unsourced "80–150ms"):**
...
**CAP posture under authority partition (decision, with published numbers — resolves DS-crit availability math):**
...
### 4.5 Revocation by *reason*, not just tier (resolves SEC-high-revocation-allow-window)
...
### 4.6 Skew & ordering (resolves DS-med-clock-skew)
...
### 5.1 Canonical module map (resolves MOD-low-module-map-artifact)
...
### 5.4 Extraction trigger (concrete, not a vibe — resolves DS-med-monolith-extraction-trigger)
...
### 5.5 Bounded kernel — checkable fitness function (resolves MOD-med-kernel-eviction-rule)
...
### 5.6 TierLattice: additive, safe-by-construction (resolves DS-high-tierlattice-barrier, MOD-crit-lattice-evolution, SEC-low-lattice-reclassification)
...
### 5.8 Swappable AI router & retrieval via interfaces (resolves MOD-high-router-swappability, MOD-med-retrieval-swappability)
...
### 7.4 Cell-local SpiceDB: go/no-go fork, not a fallback (resolves DS-high-spicedb-replication)
...
- BYO is constrained to a **pre-certified matrix (2–3 clouds, 2–3 providers)** in early phases and priced as a higher-touch edition with professional-services attach (resolves GTM-med-byo-cost).
...
### 8.5 Guardrails
Output-channel egress check (§11.5: completions grounded in tainted sources re-checked against requester attributes), prompt-injection filtering on retrieved context, tier-pinned tool use, PEP-on-every-agent-action (Phase 3 multi-agent, no DPoP key).
...
### 9.4 Discovery correctness under partial failure (resolves DS-low-noise-correctness)
...
### 10.1 Orchestration substrate ownership (one-line rules — resolves MOD-med-orchestrator-sprawl)
| Substrate | Owns ONLY | 
|---|---|
...
**Rule committed to the contracts doc; new background work is gated against it in review.** **Decision: Temporal is deferred — Phase 1 grant-lifecycle uses a Dagster sensor + Temporal only when a customer contract requires durable compensation**, deferring a whole substrate (resolves the "three orchestrators" concern).
...
### 10.3 Cell↔Exchange sync: CDC consistency contract (resolves DS-high-cdc-semantics)
...
### 11.1 Classifier abstention = quarantine, Phase 0 (resolves SEC-crit-classifier-default)
...
### 11.2 Central-index privacy bound proven BEFORE real embeddings ship (resolves SEC-crit-index-one-way-door)
The index is a **one-way door** (published embeddings cannot be unpublished). **We move the membership-inference + embedding-inversion + cross-tenant-linkage red-team to a Phase-1 GATE on the index DESIGN — proven on SYNTHETIC/de-identified corpora BEFORE any real embedding is written.** (v4's Phase-2 GA gate was past the point of no return.)
...
### 11.3 Confidential-tier crypto & key lifecycle (resolves SEC-med-key-lifecycle)
...
### 11.4 Audit: anchored against the node operator (resolves SEC-high-audit-operator-rewrite, DS-med-serial-audit-chain)
...
### 11.6 Export control: operational-access surface, not just end-users (resolves SEC-med-deemed-export-operational)
...
### 11.7 GDPR cross-border transfers (resolves SEC-med-gdpr-transfer)
...
**SOC2 Type I timing is a function of buyer demand, NOT a fixed Phase-0 commitment (resolves COST-high-soc2-cash).** The Phase-0 sandbox/de-identified pilot is structured to avoid the full prod review; SOC2 Type I is targeted at the **Phase-0/1 boundary** (deferrable if the sandbox pilot doesn't require it, saving Phase-0 cash — validated with a real CISO, Q17). SOC2 Type II → Phase 2; ISO 27001 → Phase 3 (EU).
...
### 13.2 Managed vs self-host (resolves COST-high-self-host)
...
### 13.3 Hot-tenant cell-split protocol (resolves DS-med-hot-tenant-cell-split)
...
### 14.2 Central index/graph growth model — costed in the body (resolves DS-high-central-growth-uncosted)
...
| PSI scaling (resolves DS-low-PSI) | **Phase-1 usability spike**; pairwise PSI is **O(N²)** across the federation — bounded set sizes; **documented fallback: k-anonymized bucketed overlap via the central index** so the feature is NOT architecturally dependent on cryptographic PSI |
...
### 15.2 Testing strategy (resolves MOD-high-testability)
...
- **Confidential-path tests run against a vLLM container in CI** (not Ollama), and against cloud GPU staging — never the M4 Max (resolves COST-med-gpu-parity).
...
### 16.1 Per-service monthly floor table (resolves COST-med-vendor-floors, DS-high-cogs-vs-growth)

| Service | Idle floor (N=1) | N=5 | Notes |
|---|---|---|---|
| Managed cell compute + storage | ~$1.2k/mo | ~$5k/mo | |
| vLLM GPU (confidential inference) | ~$1.5k/mo (shared/burstable) | ~$4k/mo | **Phase-0 uses shared/burstable GPU**, not dedicated, to collapse per-tenant fixed cost (decision below) |
...
| **CloudHSM (Revocation Authority)** | **$0 (Phase 0)** | **~$4k/mo (3-region, Phase 2)** | **multi-region HSM authority is PHASE 2; Phase-1 uses a single-region authority (accept SPOF for the dyad)** — resolves the COGS contradiction |
| KMS | ~$0.1k/mo | ~$0.3k/mo | |
| **Phase-0 COGS/cell** | **~$1.5–2.5k/mo (shared infra)** | | **below the $30k v4 figure pitfall** |

### 16.2 Unit economics reconciled (resolves COST-crit-unit-economics, GTM-high-30k-floor)
**The v4 contradiction (floor price $30k < per-cell COGS $36–60k) is resolved by BOTH levers:**
...
2. **Collapse Phase-0 per-tenant COGS to ~$1.5–2.5k/mo via shared/burstable GPU + shared index** (the dedicated-idle-OCU-from-node-1 decision is **reversed for Phase 0**; dedicated cell reserved for the largest hot tenant only, §13.3).
...
- **Phase-0 is explicitly a funded-loss validation phase covered by seed** until logos × ACV clears burn (revenue model below).

### 16.3 Phase-0 revenue model vs burn (resolves COST-crit-unit-economics, COST-low-burn-breakdown)
- **Burn breakdown (resolves the 60%-spread):** 3 founding eng @ ~$18k/mo loaded = $54k; GTM founder $16k; fractional GRC + counsel $12k; managed-service + GPU floors $6k; **compliance cash line (SOC2 auditor, pen-test, Vanta/Drata) ~$6k/mo amortized** (resolves COST-high-soc2-cash). **≈ $94k/mo** (tightened from the $80–130k range).
...
### 16.4 Pilot→recurring conversion (resolves GTM-med-gtm-capacity)
...
| **Fractional GRC + counsel** | **capacity-ledgered (resolves COST-med-grc-capacity):** FERPA/GDPR/ITAR review + DPA/DUA/SCC templates ≈ 20 hrs/wk; **HECVAT pre-fill NOT assigned to the GTM founder**; SOC2 prep scoped to demand (§11.9) | — |

---

## 17. Risks & Mitigations

| # | Risk | Type | Mitigation (v5) |
|---|---|---|---|
...
| R17 | Defensibility race lost | Market | **Testable moat (data-gravity at N=2 + network effect + cert depth); incumbent time-to-replicate estimated; defensible-N defined below; regulated-niche down-scope is PRIMARY plan-B with triggers (§1, GTM-med)** |
| R18 | Incumbent enters | Market | **NOT resting on inaction: data-gravity + certification depth raced; quantified replicate window (§1)** |
| R19 | Phase-0 undifferentiated | Market | **Two-gate validation (Gate B for federation thesis); ONE cheap federation differentiator in Phase 0 = cross-institution PUBLIC-tier expert discovery over OpenAlex (no confidentiality machinery) (§3.3, COST-high)** |
